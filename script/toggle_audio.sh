...
if [ "$#" -eq 1 ]; then    
    if [ "$1" = "analog" ]; then
        ...
        SND_PROFILE=`pacmd list-cards | grep "output:analog.* available: no" | head -n1 | awk -F" " '{print substr($1, 1, length($1) -1)}'`
        ...
        SND_PROFILE=`pacmd list-cards | grep "output:hdmi.* available: unknown" | head -n1 | awk -F" " '{print substr($1, 1, length($1) -1)}'`
